...
class ChartServiceTests: XCTestCase {
    ...
    func testAddOneProductToChart() throws {
        ...
        XCTAssertEqual(chartService.add(Product(id: UUID(), name: "Name", description: "Desc", price: 0.0)).products.count, 1)
        XCTAssertEqual(chartService.chart.products.count, 1)
    }
    
    func testAddTwoProductToChart() throws {
        let products = [Product(id: UUID(), name: "Name", description: "Desc", price: 0.0),
                        Product(id: UUID(), name: "Name", description: "Desc", price: 0.0)]
        let chartService = ChartService(Chart(products: []))
